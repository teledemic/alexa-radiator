'use strict';

let EnglishStrings = {
    "WELCOME_MSG": "Welcome to {{ skillName }}",
    "HELP_MSG": "Welcome to {{ skillName }}. You can play, stop, resume listening.  How can I help you ?",
    "UNHANDLED_MSG" : "Sorry, I could not understand what you've just said.",
    "CAN_NOT_SKIP_MSG" : "This is live radio, you have to wait for next track to play.",
    "RESUME_MSG" : "Resuming {{ skillName }}",
    "NOT_POSSIBLE_MSG" : "This is live radio, you can not do that.  You can ask me to stop or pause to stop listening.",
    "STOP_MSG" : "Goodbye."
};
module.exports = {
    "en-US": {
        "translation": EnglishStrings
    }
};
